function preprocess_data_single(basedir, target_subject)
% PREPROCESS_DATA_SINGLE Modified version to process only one subject
% Usage: preprocess_data_single('p:\Research\AAD', 'S1.mat')

if nargin < 1
    basedir = pwd;
end
if nargin < 2
    target_subject = 'S1.mat';  % Default to S1.mat
end
...
%% Preprocess EEG for single subject only
...
% Process only the target subject
subject_file = fullfile(basedir, target_subject);
if ~exist(subject_file, 'file')
    error('Subject file %s not found', subject_file);
end

fprintf('Processing subject: %s\n', target_subject);
loaded_data = load(subject_file);
...
save(fullfile(preprocdir,target_subject),'preproc_trials')
